...
`/finish_mapping` first calls the mapper save service and then runs
`finalize_map.py --replace-raw` while ROS is still active. Wait for its success
response before stopping the mapping launch. Do not expect
`FAST_LIO/PCD/scans.pcd` to be generated.
