...
- The development document must cover every required functional package:
  chassis/CAN, Livox, FAST-LIO, system bringup, TF, Bayesian point-cloud map,
  map conversion, cloud/pose adapters, NDT localization, navigation, D435i,
  global-planning test, and navigation logging. For each, distinguish upstream
  integration from project-owned implementation and give dependencies, files,
  build target, launch command, verification, and failure boundary.
